...
- When all thesis candidates are simultaneously above fair value, the correct response is to advance research on watchlist names — not to wait passively. A thesis must exist before a buy can happen; if a name enters the buy zone without a ready thesis, that entry is a guaranteed miss. Periods of broad market weakness (multiple names down 20–40% over six months, trading below their 200-day SMA) are the natural time to write new theses, because the research will be done before prices arrive. After 39 calendar days with only 3 theses on file and all 3 above fair value, the portfolio has no path to deployment — and that is a research-pipeline failure, not disciplined patience. — 2026-07-25
...
## Changelog
- 2026-07-25 — Added lesson: when all thesis candidates are above fair value simultaneously, advance research on watchlist names; broad market weakness is the time to write new theses, not to wait passively.
